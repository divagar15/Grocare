...
global $util;
	...
	{
		$util->info_option_msg('SEO Redirection is <b>disabled for admin</b> only, you can go to option tab and enable it!');
	}
...
function SR_HideMessageAjaxFunction()
	{  
		add_option( 'sr_upgrade_message','yes');
	}  


function SR_after_plugin_row($plugin_file, $plugin_data, $status) {
	        
			if(get_option('sr_upgrade_message') !='yes')
			{
