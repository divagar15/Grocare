jQuery( document ).ready( function( $ ) {
	...
	$api_key.bind( 'input', function() {
		...
		var data = {
			...
			'getresponse_api_key' : $api_key.val()
		};
	...
	function getresponse_toggle_fields() {
       ...
       // toggle send_opt_int_bait fearure once getresponse double opt in is selected
       $( 'select[name="fca_eoi[getresponse_double_opt_in]"]' ).select2().on("change", function(e) {
//          if( $( 'select[name="fca_eoi[getresponse_double_opt_in]"]' ).val() == 'false' ) {
