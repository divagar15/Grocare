...
{	
	...
	public function addContact($campaign, $name, $email, $action = 'standard', $cycle_day = 0, $customs = array())
	{
        ...
        if (!$this->isValidIP($ip)) $ip = '127.0.0.1';

		$params = array('campaign' => $campaign, 'action' => $action,
						'email' => $email, 'cycle_day' => $cycle_day, 'ip' => $ip);
	...
	private function execute($request)
	{
        ...
        if ($response->error && FCA_EOI_DEBUG ) {
            echo 'Whoops, something is broken: <br>';
            echo '<h2>'.$response->error->message.'</h2>';
            wp_die();
            }
