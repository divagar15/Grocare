...
function campaignmonitor_object_create( $settings ) {
	...
	$api_key = K::get_var('campaignmonitor_api_key', $eoi_form_meta, $suggested_api );
	$client_id = K::get_var('campaignmonitor_client_id', $eoi_form_meta, $suggested_id );
...
function campaignmonitor_integration( $settings ) {
	...
	$selected_list =	 empty($fca_eoi['campaignmonitor_list_id']) ? $suggested_list : $fca_eoi['campaignmonitor_list_id'];
	...
	K::fieldset( campaignmonitor_string( 'Form integration' ) ,
		array(
			array( 'input', 'fca_eoi[campaignmonitor_api_key]',
				array(
					'class' => 'regular-text',
					'value' => K::get_var( 'campaignmonitor_api_key', $fca_eoi, $suggested_api ),
				),
			...
			array( 'input', 'fca_eoi[campaignmonitor_client_id]',
				array(
					'class' => 'regular-text',
					'value' => K::get_var( 'campaignmonitor_client_id', $fca_eoi, $suggested_id ),
				),
