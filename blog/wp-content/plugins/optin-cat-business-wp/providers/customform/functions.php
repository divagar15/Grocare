...
function customform_add_user( $settings, $request_data ) {
	...
	$request = $request_data['customform_request'];
	
	if ( !empty ( $request ) ) {
		$sender = new EasyOptInsCustomFormSender( $request, $request_data );
		$result = $sender->send();
	} else {
...
class EasyOptInsCustomFormSender {
	...
	public function send() {
		$response = wp_remote_get( $this->get_url(), $this->get_options() , array( 'timeout' => 15 ) );
		if ( wp_remote_retrieve_response_code( $response ) === 302 ) {
			$location = wp_remote_retrieve_header( $response, 'location' );
			if ( $location ) {
				$response = wp_remote_get( $response['headers']['location'], array_merge( array(
	...
	private function options_for_multipart_post( $options ) {
		...
		$options['headers']['Content-Type'] = 'application/x-www-form-urlencoded; boundary=' . $boundary;
...
class EasyOptInsCustomFormParser {
	...
	private function parse_multipart() {
		...
		return ! empty( $enctype ) && strtolower( $enctype ) === 'application/x-www-form-urlencoded';
	}
