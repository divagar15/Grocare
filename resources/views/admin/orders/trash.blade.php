...
@section('content')     	
<div class="page-header"><h1>Trash Orders</h1></div>
                        ...
                        <form id="filter-form" class="form-horizontal" role="form" method="post">
                                  ...
                                  <div class="form-group" style="margin-top:20px;">
                                    <div class="col-sm-7 col-sm-offset-5">
                                      ...
                                      <a href="{{URL::to('admin/orders/trash')}}" class="btn btn-default btn-sm">Cancel</a>
                                    </div>
                                  </div>
...
<!--                                       <input type="checkbox" name="checkOrder[]" id="checkOrder" value="{{$detail->id}}" />
 -->                                      </td>
                                      ...
                                      <td>
                                          <a class="btn btn-xs btn-info" href="{{URL::to('admin/orders/trash/'.$detail->id)}}">View</a> 
                                          <a class="btn btn-xs btn-success successfulOrder" data-id="{{$detail->id}}" href="javascript:void(0)">Move to Successful Order</a> 
                                          <a class="btn btn-xs btn-danger deleteOrder" data-id="{{$detail->id}}" href="javascript:void(0)">Delete Permanently</a> 
                                      </td>
                                    </tr>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
          $(document).on("click",".deleteOrder",function() {
            var id = $(this).data('id');
            var confirmMsg = confirm('Are you sure want to move this order permanently?');
            if(confirmMsg) {
              window.location.href = "{{URL::to('admin/orders/delete-permanently')}}/"+id;
            }
          });
          ...
          $(document).on("click",".successfulOrder",function() {
            ...
            if(confirmMsg) {
              window.location.href = "{{URL::to('admin/orders/trash-successful')}}/"+id;
            }
          });
